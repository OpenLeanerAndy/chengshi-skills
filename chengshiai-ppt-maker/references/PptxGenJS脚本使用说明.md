...
- PowerPoint、LibreOffice 或 WPS 不是生成文件的必需条件，但至少具备其中一种时才能完成页面渲染和视觉验收。使用 LibreOffice 渲染时还需要同时安装 `pdftoppm`。
...
表格页经用户确认拆分（`splitMode: "paginate"`）后，脚本会把续页标题生成为“原标题（续N）”。对应的`revisions[].effectivePages`必须包含这些连续页，且标题与生成结果一致（含“（续N）”后缀），否则结构一致性检查会失败。

## 页面类型

### `title`

标题页。可提供 `title`、`department` 和 `date`；缺省时使用顶层同名字段。主标题超过15个字时，应在输入 JSON 中按语义完整性手动拆为两行（用 `\n`）；脚本内置的自动缩小只是兜底，不能代替拆行。
...
| `type` | 关键字段 | 说明 |
|---|---|---|
| `text` | `text` | 普通段落，按14号字渲染。 |
| `subtitle` | `text` | 模块内子标题：深蓝色、加粗、居中，下方带同色下划线，14号字。 |
| `bullets` | `items` | 项目符号数组；每项可为字符串，或带 `text`、`level` 的对象，按14号字渲染。 |
...
- 字号约定：`text`和`bullets`正文按14号字渲染并估算高度；表格、事项矩阵子项和提示块正文按10号字渲染并估算行高。两者均不得低于[风格定义规则](./风格定义.md)的字号下限。
- 表格中的小结、合计行可通过单元格对象的 `options` 覆盖默认样式实现，例如 `{"text":"合计","options":{"bold":true,"color":"FFFFFF","fill":{"color":"4472C4"}}}`；文字内容仍须原样保留。
